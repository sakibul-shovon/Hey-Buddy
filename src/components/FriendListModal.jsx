...
const FriendListModal = ({ isOpen, onClose, friends = [] }) => { // Default to an empty array
        ...
        <ul className="space-y-3">
          {friends.map((friend) => (
            <li key={friend._id} className="flex items-center space-x-3">
              <img src={friend.profilePicture?.url || "/default-profile.png"} alt={friend.name} className="w-10 h-10 rounded-full" />
              <div>
